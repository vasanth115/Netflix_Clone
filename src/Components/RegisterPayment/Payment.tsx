...
const Payment = () => {
  const { setIslogin , setpack , setpackage } = useFormStore();
  const Navigate = useNavigate();

  const HandleClick = (e: React.MouseEvent<HTMLDivElement, MouseEvent>, item: any) => {
  ...
  return (
    <div className='payment'>
      <div className='payment__container'>
        ...
        <Link to='/' className='payment__btn'>
          Sign Out
        </Link>
      </div>
      ...
      <div className='payment__select'>
        {Data.map((item) => (
          <div
            className='payment__select__item'
            key={item.id}
      ...
      <p className='payment__paragraph'>
        ...
        capabilities. Not all content is available in all resolutions. See our <a href=''>Terms of Use</a> for more
        details.{' '}
      </p>
      <p className='payment__paragraph'>
