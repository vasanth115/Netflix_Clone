...
const UPIPay: React.FC = () => {
    ...
    const handleClick = () => {
        if (!number) {
            alert("Fill the UPI ID");
        } else if (number.length !== 10) {
            alert("Provide a Correct UPI ID");
        } else {
            setValue(true);
        }
    }
    ...
    return (
        <div className='upipay'>
            <div className="upipay__header">
                ...
                <Link to='/' className='upipay__link'>Sign Out</Link>
            </div>
            {
                !value ? <div className="upipay__container">
                    <span className='upipay__span'>STEP 3 OF 3</span>
                    <h1 className='upipay__title'>Set up UPI AutoPay</h1>
                    <p className="upipay__paragraph">You can change this recurring payment any time in your settings.</p>
                    <div className="upipay__inputs">
                        ...
                        />
                        <button className='upipay__button' onClick={handleClick}>Next</button>
                    </div>
                </div> : <div className='upipay__timer'>
                        <p className='upipay__para'> Request is Sent to your UPI ID Confirm Your Payment within the Time</p>
                        <h1 className="upipay__timeleft">Time left: {Math.floor(timeLeft / 60)}:{timeLeft % 60 < 10 ? `0${timeLeft % 60}` : timeLeft % 60}</h1>
                    </div>
            }
            <SignUpFooter />
        </div>
